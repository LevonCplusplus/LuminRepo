...
signals:
    void loginResult(Status status,const QString& message,const QString& mail = nullptr,const QString& session = nullptr);
    void verifyResult(Status status,User user = User());
    void tokenRefreshed();
    void authExpired();
    void refreshFailed();
private:
